...
DriveSubsystem::DriveSubsystem()
    : Subsystem("DriveSubsystem"),
      drive(new RobotDrive(
          RobotMap::leftDrivePorts[0], RobotMap::leftDrivePorts[1],
          RobotMap::rightDrivePorts[0], RobotMap::rightDrivePorts[1])) {
  shifting = new DoubleSolenoid(RobotMap::lowGear, RobotMap::highGear);

  encLeft = new Encoder(RobotMap::leftDriveEncoders[0],
                        RobotMap::leftDriveEncoders[1]);

  encRight = new Encoder(RobotMap::rightDriveEncoders[0],
                         RobotMap::rightDriveEncoders[1]);
}
...
void DriveSubsystem::ShiftGear(bool high) {
  currentGear = high;
  shifting->Set(high ? DoubleSolenoid::Value::kForward
